...
\begin{align}
    ...
    + i \bar{\tilde{\psi}}_i^\beta \slashed{D} \tilde{\psi}^i_\beta\nonumber\\&+\tr((D^\mu\Phi)(D_\mu\Phi))+ (y^\beta_\alpha \tilde\psi_\beta \Phi\psi^{\alpha}+\mathrm{h.c.})+\lambda_1\tr(\Phi^2)^2+\lambda_2\tr(\Phi^4)
    \,.\label{eq:Ladj}
\end{align}
...
From Eq.~\eqref{eq:Ladj} we see that the structure of the Yukawa term is changed, compared to the previous model. It no longer includes the chiral fermionic sector, but instead couples the vector-like fermions. Similar to the model in Section \ref{sec:fundscalarNg}, the Yukawa coupling is a matrix since the product of $\tilde\psi$ and $\psi$ has dimensions $p\times (N\mp 4+p)$, since they transform under two different global groups. Analogously to the cases of Section \ref{sec:fundscalarNg}, we can decompose the matrix in terms of $p$ independent real couplings. With this, we find the beta functions of the model to be 
\begin{align}
    \beta_g&=-\frac{1}{3}\,\alpha_g^2\left(17N \pm 12 -4p\right)\,,\\
    \beta_{y}&=\alpha_y\left[\left(\frac 6N-6N\right) \alpha_g+ 2p \alpha_{y}+ \left(N-\frac 3N\right)\alpha_y\right]\,, \label{eq:betaYadj}\\
    \beta_{\lambda_1} &=4\frac{N^2-9}{N}\alpha_{\lambda_1}^2+12\alpha_{\lambda_1}\alpha_{\lambda_2}+4(p\alpha_{y}-3\alpha_{g})\alpha_{\lambda_1}+3N\alpha_g^2-2p\alpha_{y}^2\,,\label{eq:betalamadj1}
    \\
    \beta_{{\lambda_2}}&=(7+N^2)\alpha_{\lambda_2}^2+\frac{12(3+N^2)}{N^2}\alpha_{\lambda_1}^2+\frac{4(2N^2-3)}{N}\alpha_{\lambda_1}\alpha_{\lambda_2}+4(p\alpha_{y}-3N\alpha_{g})\alpha_{\lambda_2}+18\alpha_{g}^2\,.\label{eq:betalamadj2}
\end{align}
 \begin{table}[ht]
    \centering
...
For the GG models, a result of the roots analysis can be found in Figure~\ref{fig:roots_fixed} for all couplings on fixed flow. There are three possible scenarios of real or complex roots, depending on the parameters $N$ and $p$, where the viable region in green is delimitated by the condition $\Delta=0$ (see Eqs. \eqref{eq:Delta0noY} and \eqref{eq:Delta0}), where $\Delta$ is the discriminant of the fourth degree polynomial in Eq.~\eqref{eq:fourthdegpoly}. This region should be compared with the asymptotic freedom condition for the gauge coupling running, which gives an upper bound on $p$ from the condition $b_0=0$
\begin{align}
    p_{\max}(N)&=\frac{17N\pm12}{4}\,, \label{eq:pmaxadj}
\end{align}
shown as a dashed line in the plots. Thus, there is no viable region on fixed flow, since there is no green region below the dashed line (representing this condition) in Figure~\ref{fig:roots_fixed}.

Conversely, in the off-fixed-flow regime the UV behaviour of the theory may then be analysed by solving the gauge–scalar subsystem on fixed flow, effectively reducing the quartic beta equations to
\begin{align}
...
\subsection{Inclusion of Chiral Families}\label{sec:adjNg}
 Let us now upgrade the GG and BY model with an adjoint scalar to include chiral generations. As done in Section \ref{sec:fundscalarNg}, the global symmetry group will depend on the number of families: $\mathrm{SU}(N_g(N\mp4)+p)$. This also changes the transformation table, see Table~\ref{tab:adj_Ng}. 
...
Since, the Yukawa interaction does not include the chiral fermions in the case of an adjoint scalar, only the gauge beta function will be affected by the number of generations as follows
\begin{equation}
     \beta_g=-\frac{1}{3}\left(21N \pm 12 N_g-4p-4N_gN\right)\alpha_g^2\,,
\end{equation} 
while the Yukawa and quartic beta functions remain the same as for the one-family models (see Eqs. \eqref{eq:betaYadj}, \eqref{eq:betalamadj1} and \eqref{eq:betalamadj2}).
\begin{figure}[t!]
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
...
It is only possible to express the upper boundary in a compact analytic form, analogous to the case of a single chiral family, from the $b_0=0$ condition, which yields
\begin{align}
    p_{\max}(N,N_g)=\frac{21N\pm12 N_g-4NN_g}{4}\,.\label{eq:padjscalar}
\end{align}
The lower boundary is described by $\Delta=0$ (see Eqs. \eqref{eq:Delta0noY} and \eqref{eq:Delta0}).

Now we can perform a CAF analysis. First, consider the GG model with $N_g=3$. As shown in Figure~\ref{fig:Ng2}, the result mirrors that of the one family case: CAF solutions only exist on fixed flow. More in detail, Figure~\ref{fig:roots_fixed_ng2} illustrates that on fixed-flow, all the roots will be complex within the region where the gauge coupling is asymptotically free. In Figure~\ref{fig:final_combined_ng2}, the light blue region represents the CAF region for the Yukawa coupling off fixed flow. Solutions only exist for $N\geq 7$ and $p\geq 24$, once again excluding the $\mathrm{SU}(5)$ GUT model.
...
\begin{figure}[t!]
    ...
    \label{fig:GGadjNG37}
\end{figure}
Matters change when the number of families is $N_g=5$, as illustrated in Figure~\ref{fig:roots_fixed_ng3}, which displays the nature of the roots. We identify two regions with two regions with two real and two complex roots. One of them is excluded by the requirement of asymptotic free gauge coupling. Thus, only one region with an asymptotically free gauge coupling remains. Furthermore, the polynomial has one sign change in this area, and thus has one positive real root as a consequence of Descartes' rule of signs. As shown in Figure~\ref{fig:final_combined_ng3}, the GG model with $N_g=5$ has a CAF region on fixed flow highlighted in dark blue and starting at $N \geq 13$.

Off fixed flow, the situation is similar to that of lower $N_g$: a viable region exists below the dashed line, as shown by the light blue region in Figure~\ref{fig:final_combined_ng3}. Similar solutions can be found for larger $N_g$, as illustrated in Figure~\ref{fig:gg_combined_ng5} and \ref{fig:gg_combined_ng6} for $N_g=6,7$, respectively. Note that no CAF regions are found for $\SU(5)$. Finally, for $N_g>13$, the gauge coupling loses asymptotic freedom.

For the BY models, the gauge coupling looses asymptotic freedom for $N \geq 5$, as was established in Section \ref{sec:fundscalarNg}. In Figure~\ref{fig:GGadjNG37}, the CAF region can be seen for the case of $N_g\in\{1,2,3,4\}$.
