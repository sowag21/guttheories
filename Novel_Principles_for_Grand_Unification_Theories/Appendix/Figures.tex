...
\end{equation}
using this, we can write a general matrix $M$ as a linear combination of the identity matrix and the generators of $\SU(N)$ as
\begin{equation}
      M=n_0 \mathbb{1}+n_aT^a\,,
\end{equation}
where $n_0$ and $n_a$ are coefficients. Now let us take the trace on both sides and simplify using that $\tr(T^a)=0$ and $\tr(\mathbb{1})=N$
\begin{equation}
      \tr(M)=n_0 \tr(\mathbb{1})+n_a\tr(T^a)=n_0N\,.\label{eq:tr1}
\end{equation}
Next, using the definition of the normalisation of the generators
\begin{equation}
      \tr(MT^b)=n_0 \tr(\mathbb{1}T^b)+n_a\tr(T^aT^b)=\frac{n_b}{2}\,.\label{eq:tr2}
\end{equation}
where $\tr(T^aT^b)=\frac{1}{2}\delta^{ab}$.

Now using Eq.~\eqref{eq:tr1} and \eqref{eq:tr2}, the constants $n_0,n_a$ can be replaced, and the matrix $M$ can be written as
\begin{equation}
      M=\frac{\tr(M)}{N} \mathbb{1}+2\tr(MT^a)T^a\,,
\end{equation}
the matrix decomposition can be written in index notation
\begin{equation}
      M_{ij}=\frac{\tr(M)}{N} \delta_{ij}+2\tr(MT^a)T^a_{ij}\,.
\end{equation}
Using that it is in Euclidean space with the metric $\delta_{ij}$, the traces can be simplified, and the LHS can be rewritten as
\begin{equation}
...
\end{equation}
Isolating the generators, we obtain the Fierz Identity 
\begin{equation}
...
The scalar beta function computed strictly for $N=3$ for the BY model (one quartic term), is given by
\begin{align}
...
\end{align}
The original two beta functions with $N=3$, where the general two quartic terms have written, we have 
\begin{align}
...
\end{align}
Now we can add the two beta functions, and let $\alpha_{\lambda_1}+\alpha_{\lambda_2}=\alpha_{\lambda}$, re-obtaining Eq.~\eqref{eq:N3by}:
\begin{align}
...
\end{align}
Thus, the two beta functions simply add, and it has been found numerically that both results give no CAF regions for the BY model with $N=3$. Thus, the analysis with two quartic terms in the main text is valid, also for $N=3$. 
...
\label{fig:flowgy}
\end{figure}
To better understand the CAF regions, we can analyse the RG flow of the BY model with one anti-fundamental scalar (a similar analysis applies to the GG model). Note that in all the flow diagrams, the arrows point towards the UV.
...
In the model described in Section \ref{sec:adj}, we obtain Figure~\ref{fig:DeltaadjGG} and \ref{fig:gg_combined_ng1}. Similarly to  the GG model, only the case of the Yukawa coupling off fixed flow yields a CAF region this can be seen by the root signature in Figure~\ref{fig:DeltaadjGG}, where only \ref{fig:roots_noyGG} has real roots below the dashed line. 

\begin{figure}[h!]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figures/CAFplot_1_RootsFixed_Ng1.pdf}
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figures/CAFplot_2_RootsNoY_Ng1.pdf}
        \caption{$\alpha_y$ off fixed flow} 
        \label{fig:roots_noyGG}
    \end{subfigure}
    
    \caption{ Root structure of the fourth-degree polynomial for the BY model, for (a) all couplings on fixed flow and (b) the Yukawa coupling off fixed flow. The shaded regions denote four complex roots (white), four real roots (dark green), and two real and two complex roots (light green). The dashed black line illustrates the $b_0=0$ condition.} 
