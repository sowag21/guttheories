...
\section{Solutions to ODE's}\label{app:caf}

The computations of the solutions to the quartic ODE, is given here. It's beta function can be written as
\begin{equation}
...
\end{equation}
Recall that the Yukawa coupling can be asymptotically free in two cases: on or off fixed flow with the gauge coupling. We consider the later here. 
...
\begin{align}
    ...
    \sqrt{-k} \tan \bigg( \frac{1}{2} \bigg( \frac{\sqrt{-k} \ln (\alpha_{g})}{b_0} 
    \\-&\sqrt{-k} \bigg( 2b_0 \sqrt{-k} \tan^{-1} \bigg( \frac{(b_0 - d_2) \alpha_{g} \sqrt{-k} - 2d_1 \sqrt{-k} \alpha_{\lambda_0}}{k \alpha_{g_0}} \bigg) + k \ln (\alpha_{g_0}) 
    \bigg)\bigg) \frac{1}{b_0 k} \bigg) \Bigg]\,,\nonumber
\end{align}
where the definition of $k_0$ can be used to simplify the expression, we obtain:
\begin{equation}
    \boxed{\alpha_{\lambda} = \frac{\alpha_{g}}{2d_{1}} \left[ b_0 - d_2 + \sqrt{-k} \tan\left( \frac{\sqrt{-k}}{2b_0} \ln\frac{\alpha_g}{\alpha_{g_0}} - \tan^{-1}\frac{k_0}{\sqrt{-k\alpha_{g_0}}} \right) \right] , \quad k<0 \,.}
...
\begin{align}
    \alpha_{\lambda}(\alpha_g) = \frac{\alpha_g}{2\alpha_1} \left[ b_0 - d_2 + i\sqrt{k} \tan \left( \frac{i\sqrt{k}}{2b_0} \ln \left( \frac{\alpha_g}{\alpha_{g_0}} \right) + \tan^{-1} \left( \frac{k_0}{i\sqrt{k} \alpha_{g_0}} \right) \right) \right]\,.
\end{align}
Using the identities \(\tan(iz) = i\tanh(z)\) and \(\tan^{-1}(iz) = i\tanh^{-1}(z)\), we rewrite the argument of the tangent. Let
\begin{align}
z = \frac{\sqrt{k}}{2b_0} \ln \left( \frac{\alpha_g}{\alpha_{g_0}} \right) + \tanh^{-1} \left( \frac{k_0}{\sqrt{k} \alpha_{g_0}} \right)\,.
\end{align}
...
where
\begin{align}
A = \frac{\sqrt{k}}{2b_0} \ln \left( \frac{\alpha_g}{\alpha_{g_0}} \right), \quad 
...
\begin{align}
z &= -\tan^{-1}\left( \frac{k_0}{\epsilon\alpha_{g_0}} \right) + \frac{\epsilon}{2b_0} \ln\frac{\alpha_g}{\alpha_{g_0}} \,.
\end{align}
...
\begin{align}
    ...
    \nonumber\\&=\frac{\alpha_g}{2d_1} \left[ \frac{\frac{4b_0d_1\alpha_{\lambda_0}}{\alpha_{g_0}}+\left(\frac{-2d_1\alpha_{\lambda_0}}{\alpha_{g_0}} + b_0 - d_2\right)\ln \frac{\alpha_g}{\alpha_{g_0}}(b_0-d_2)}{{2b_0} + \left(\frac{-2d_1\alpha_{\lambda_0}}{\alpha_{g_0}} + b_0 - d_2\right) \ln \frac{\alpha_g}{\alpha_{g_0}}} \right]\,,\label{eq:appk0}
\end{align}
...
\end{equation}
We have now obtained the last of the analytic solutions to Eq.~\eqref{eq:betalamfracapp}.


\section{CAF Analysis Using the Fixed-Flow Framework} 
...
\subsection{Two Quartic Couplings}\label{sec:appCafadj}
Using Eq.~\eqref{eq:solCAF2app} we can write the polynomial equation for the quartic scalar coupling as two coupled quadratic equations, where the relevant case for our models has $b=0$:
\begin{equation}
...
\end{equation}
where the coefficients are defined in Eq.~\eqref{eq:coef2quartic}.

To determine whether there is any CAF region in this case, we must find the roots of the functions. This is done by inserting  ${z}$ from Eq.~\eqref{eq:poly1} into Eq.~\eqref{eq:poly2} and multiplying by $(d{x})^2$
\begin{equation}
...
\begin{align} \Delta=&- B^4 D^2 N^2 (1 + N^2)^2 P_3^2 + B^2 D^3 N (7 + N^2) P_3^2 \nonumber\\ & - 64 C B^6 N^3 (1 + N^2)^3 P_3^3 + 72 C B^4 D N^2 (1 + N^2) (7 + N^2) P_3^3 \nonumber\\&+ 432 C^2 B^4 N^2 (7 + N^2)^2 P_3^4  + B^2 D^3 N (1 + N^2)^2 P_4  - D^4 (7 + N^2) P_4 \nonumber\\&+ 72 C B^4 D N^2 (1 + N^2)^3 P_3 P_4 - 80 C B^2 D^2 N (1 + N^2) (7 + N^2) P_3 P_4 \nonumber\\&+ 96 C^2 B^4 N^2 (1 + N^2)^2 (7 + N^2) P_3^2 P_4 \label{eq:Delta0}\\ & - 576 C^2 B^2 D N (7 + N^2)^2 P_3^2 P_4 + 432 C^2 B^4 N^2 (1 + N^2)^4 P_4^2\nonumber\\& - 576 C^2 B^2 D N (1 + N^2)^2 (7 + N^2) P_4^2  + 128 C^2 D^2 (7 + N^2)^2 P_4^2  \nonumber\\&- 3072 C^3 B^2 N (1 + N^2) (7 + N^2)^2 P_3 P_4^2 - 4096 C^4 (7 + N^2)^3 P_4^3  = 0 \,,\nonumber
\end{align}
where the polynomials are given by
\begin{align} 
    P_1 &= -18 + N^2 (-3 + 4 N_g) + 4 N (\mp3 N_g + p) \,,\quad P_2 = -3 + N^2 + 2 N p \,,\\ P_3 &= 9 - 20 N^2 + N^4 \,,\quad  P_4 = 567 + 315 N^2 - 35 N^4 + N^6 \,, 
\end{align}
and the terms $B, C,D$ is given by
\begin{align}
    B &= (-3 + N^2) \big(N (15 + 4 N_g) + 4 (\mp3 N_g + p)\big) + \big(72 + N^2 (42 - 8 N_g) - 8 N (\mp3 N_g + p)\big) p\,, \\ C &= 2 P_1^2 p - 27 N P_2^2 \,,\\ D &= 23328 N P_2^2 - 12 N B^2 - 48 (3 - 2 N^2) C + (7 + N^2) \big(N B^2 + 8 (9 - N^2) C\big)\,.
\end{align}
