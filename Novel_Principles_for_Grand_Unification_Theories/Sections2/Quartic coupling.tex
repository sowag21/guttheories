...
Like the Yukawa and gauge couplings, we can consider whether the quartic coupling can be asymptotically free on its own. In this case, the solution will be similar to that of the two other couplings
\begin{equation}
    \alpha_\lambda=\frac{\alpha_{\lambda_0}}{1-d_1 \alpha_{\lambda_0} \log \left(\frac{\mu}{\mu_0 }\right)}\;.
\end{equation}
The logarithmic term in the denominator is positive $d_1>0$. Thus, the analysis is similar to that of the Yukawa coupling. At the energy scale $\mu_L=\mu_0\exp(1/{d_1\alpha_{\lambda_0}})$ the coupling will approach a Landau pole. For $\mu>\mu_L$ the coupling will be negative, which for one quartic coupling  will be a non-physical branch, since the potential would be unbounded from below. At energies $\mu<\mu_L$, the coupling will be positive with a trivial Gaussian fixed point in the IR. Consequently, the quartic coupling cannot exhibit asymptotic freedom without the gauge coupling. 
...
\end{equation}
To find the CAF conditions in a theory with a gauge, Yukawa, and quartic coupling, it is important to consider whether the Yukawa and gauge coupling is on or off fixed flow. This gives two different differential equations to solve and thus two sets of CAF conditions.

First, we consider the case of the Yukawa coupling off fixed flow. Then the differential equation simplify to
\begin{equation}
...
\end{equation}
We can divide the solutions into three categories depending on the sign of $k = (b_0 - d_2)^2 - 4d_1 d_4$, 
...
Let us do the CAF analysis for this case. From Eq.\ \eqref{eq:alphalamkg0}, it can be seen that there are two possible fixed flows between the gauge and self-coupling. Namely, when $k_0+\sqrt{k}\alpha_g=0$ or when $k_0-\sqrt{k}\alpha_g=0$ which gives us the expression
\begin{equation}
...
After the CAF analysis, the conditions when the Yukawa and gauge couplings are not on fixed flow (derived in Eq.\ \eqref{eq:CAF1con}) are found to be
\begin{equation}
