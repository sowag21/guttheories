...
For a system with two couplings $g_i$ with $i=1,2$, the scale dependence can be visualised by multiple trajectories which changes as the energy scale changes. These trajectories are called a renormalisation group trajectory and collection of these depict the renormalisation group flow \cite{Lancaster:2014}. An illustrated example can be found in Figure \ref{fig:RGflow}. 
\begin{figure}[htbp]
    \centering
        \begin{tikzpicture}[
            scale=1.1,
        ...
        \caption{Illustrative RG trajectories in the $(g_1,g_2)$ parameter space. The black dots mark the inital coupling values, with the arrows showing the RG flow.} 
        \label{fig:RGflow}
\end{figure}
...
Now having reviewed the conceptual framework of the running of the couplings, we will follow the standard QFT approach of renormalising the parameters in the lagrangian to ensure that the physical observables, such as the S-matrix, are invariant under energy scale changes. We do this through renormalisation of the theory. Renormalisation ensures that the parameters of the Lagrangian change with the energy scale, effectively countering the effect of regularisation. Thus, the Lagrangian can be expressed as
\begin{equation}
    \mathcal{L}_{\mathrm{0}}= \mathcal{L}_{\mathrm{Renorm}}+ \mathcal{L}_{\mathrm{ct}}\label{eq:Barelag}
\end{equation} 
where $ \mathcal{L}_{\mathrm{0}}$ is the so-called \textit{bare} Lagrangian, which represent the theory's original Lagrangian containing the bare parameters which is the non-renormalised parameters. On the RHS $\mathcal{L}_{\mathrm{Renorm}}$ describes the renormalised Lagrangian, whose parameters are physical and depend on the energy scale,  and $ \mathcal{L}_{\mathrm{ct}}$ describes the necessary counterterms, which absorb the divergences. In this approach, we have already let our cutoff be sent to infinity, so how do we extract the RG flows of the parameters? We can use that the renormalisation of the Lagrangian depends on conditions at some renormalisation scale \cite{Peskin:1995ev}. Let us consider an example: a non-Abelian theory consisting of massless fermions. This theory would have the following bare Lagrangian
\begin{align}
...
\end{equation}
where \(g_0\) denotes the bare coupling constant, $\gamma^\mu$ are the Dirac matrices which obey the Clifford algebra, $f^{abc}$ is the structure constant of the concerned gauge group and the gauge indices have been omitted. The bare parameters are related to the renormalised parameters as
\begin{equation}
...
Now that we understand the motivation of the RG, we can use our knowledge to write down and equation which describes how the physical parameters of the theory will change with the energy scale, this is the so-called \textit{Callan-Symanzik equation} \cite{Callan:1970yg,Symanzik:1970rt,Symanzik:1971vw}. 
...
To better understand the new parameters $\beta$ and $\gamma$, we can consider the limit in which $\delta\to\partial$ of Eq. \eqref{eq:beta0}
\begin{align}
...
\end{align}
In the Lagrangian the third term is the vertex. We can write the renormalised vertex as
\begin{equation}
...
\end{align}
We can then consider the beta function (see Eq. \eqref{eq:betafunc}) of the coupling
\begin{equation}
    \beta(g_r) = \mu \frac{\dd g_r}{\dd\mu}\,,
\end{equation}
where $g_r$ is based on the interaction of the fermion and the gauge boson, when we consider the vertex $\Gamma_{\bar{\psi} A\psi,r}$. From now on, we will drop the subscript on the coupling $g_r = g$, we have
\begin{align}
...
\end{align}
where we used that the derivative of the bare coupling is zero $\frac{\partial}{\partial\mu} g_0 = 0$, since the bare parameters, by definition, does not change with the energy scale. 
...
\begin{align}\delta_1= \raisebox{-0.8cm}{\feynmandiagram [vertical=a to b, small] {
        a -- [photon] b ,
...
\end{align}
The diagrams in Eq. \eqref{eq:delta1} represent the vertex renormalisation. The first diagram is similar to the vertex correction in Abelian theories. The second diagram is specific to non-Abelian theories, since it includes self-interactions between the gauge bosons. Thus, $\delta_1$ will absorb the divergences of these two one-loop diagrams. In Eq. \eqref{eq:delta2} $\delta_2$ is defined to renormalise the fermion wavefunction. Lastly, Eq. \eqref{eq:delta3} represents the gauge boson wavefunction corrections, coming from four one-loop diagrams, including a fermion loop, a ghost loop\footnote{Faddeev-Popov ghosts are unphysical fields and are introduced to combat gauge redundance \cite{Peskin:1995ev}.}, and two including gauge boson self-interactions. The last diagram is generally called a tadpole diagram\footnote{This diagram can equal zero depending on the chosen regularisation scheme \cite{Peskin:1995ev}.}.
...
\end{align}
where $n_f$ is the number of different fermion fields. There are three group-theoretical (see Appendix \ref{app:GT}) constants in the equations, two of them depend on the gauge group and the representation $R$ of the field: $T(R)$ is the Dynkin index and $C_2(R)$ is the quadratic Casimir index. The factor $C_2(G)$ is the quadratic Casimir as well, but of the gauge group $G$.
...
\end{align}
where the structure of the counterterm originates for the kinetic term of fermion in the Lagrangian in Eq. \eqref{eq:LnonA}. To compute the one-loop diagram, we need the three following Feynman rules\footnote{The boson propagator is found using the Feynman-'t Hooft gauge \cite{Peskin:1995ev}.}  \cite{Peskin:1995ev}:
\begin{align}
...
\begin{align}
  -i \Sigma_{\text{1 loop}}(p^2) &=\raisebox{-0.7cm}{
    \begin{tikzpicture}
        ...
        \end{feynman}
    \end{tikzpicture}
}\\&= \int \frac{\dd^4 k}{(2\pi)^4} ig \gamma^\mu t^a \frac{i(\slashed{k} + m)}{k^2 - m^2 + i \epsilon} ig \gamma^\nu t^a \frac{-i g_{\mu\nu}}{(p-k)^2 + i \epsilon}\\
  &= -g^2\int \frac{\dd^4 k}{(2\pi)^4} \gamma^\mu t^a \frac{\slashed{k} + m}{k^2 - m^2 + i \epsilon} \gamma_\mu t^a \frac{1}{(p-k)^2 + i \epsilon}\,.
\end{align}
We can now use the group theory relation of the operators \(t^a t^a = C_2(R) \mathbb{I}\) (see Appendix \ref{app:Lie}) to simplify the expression, such that we obtain
\begin{align}
...
\end{align}
where the terms linear in $\slashed{\ell}$ vanish since it is an odd function evaluated over a symmetric boundery. 

To solve the integral, we can use the general relation \cite{Peskin:1995ev} given by
\begin{align}
...
\end{align}
where it has been used that  $\Gamma(2) = 1$ \cite{Peskin:1995ev}. The next step is to impose a renormalisation condition \comment{where does this come from, why do we choose it?}
\begin{align}
    \frac{\dd}{\dd\slashed{p}} \left[ -i \Sigma_{\text{1loop}} (p^2) \Big|_{p^2=0, m=\mu} \right] &= 0\,.
\end{align}
...
\end{align}
where the derivative removes the dependence on the mass since the $dm$ term does not depend on $\slashed{p}$. The condition also fixes the momentum and mass to $p^2 = 0$ and replacing $m=\mu$. This changes out denominator $\Delta = (1-x)\mu^2$. 

This allows us to solve for the expression of $\delta_2$
\begin{align}
...
\end{align}
To obtain the beta function, we need to add all the contributions together using Eq. \eqref{eq:delta1expr}, \eqref{eq:delta2expr}, and \eqref{eq:delta3expr}, we obtain 
\begin{align}
...
\end{align}
The next step is to regularise the expression. This is done using dimensional regularisation (unlike cutoff this method does not break gauge invariance \cite{Peskin:1995ev}). This method considers the spacetime dimensions as lower than the usual four and introducing a continuous dimension $d=4-\epsilon$. Then we take the limit of $\epsilon\to0$ to recover out four dimensional world \cite{tHooft:1972tcz}.
...
\end{align}
The remaining integral $K$ can be computed by utilising dimensional regularisation and considering the limit of $\epsilon\to0$ 
\begin{align}
...
The general one-loop gauge beta function \cite{Gross:1973ju}, when including scalars as well as fermions for, is given by
\begin{equation}
...
Now that we have derived the beta function for a gauge coupling in a non-Abelian theory with fermions, it is time to better understand the significance of the function. The beta function describes how the coupling changes with the energy scale and it is therefore useful in determining how the coupling behaves at high energies. When considering short distances, the coupling has three possible outcomes. Either the coupling goes towards infinity, which defines a Landau pole \cite{Landau:1954nau}. The coupling grows infinite at a finite energy scale, which signalling an end of the perturbative regime in this limit. The second outcome would be that the value of the coupling converges to a finite non-zero value, and lastly, that the value of the coupling converges to zero. These two cases represent \textit{asymptotic safety} \cite{Weinberg:1980gg} and \textit{asymptotic freedom} \cite{Gross:1973id,Politzer:1973fx}, respectively. Rather than breaking down at a finite scale, they yield ultraviolet complete theories by guaranteeing that the coupling remains small at arbitrarily high-energies, ensuring that perturbation theory can be applied. 

In the case of asymptotic freedom effectively reduces the theory to a free field theory, i.e.\ a non-interacting theory at short distances. To have an asymptotically free coupling, the beta function has to be negative at high energies. Therefore, as shown in Eq. \eqref{eq:betagaugeformula}, gauge bosons drive asymptotic freedom, while fermions and scalars counteract it. There are examples of two of these cases in the Standard Model of particle physics, which we will see in the next section.  
...
Next, we consider the specific case of $N=3$. This allows us to investigate the gauge coupling of QCD. The beta function is then
\begin{align}
...
\begin{figure}[ht]
   ...
   \begin{subfigure}[b]{0.49\textwidth}
       \centering
       \includegraphics[width=\textwidth]{Figures/beta_function_pretty.pdf}
   ...
   \begin{subfigure}[b]{0.49\textwidth}
       \centering
       \includegraphics[width=\textwidth]{Figures/running_coupling_pretty.pdf}
       \caption{Running of coupling}
       \label{fig:runningcoupling}
   \end{subfigure}
   \caption{\comment{redo it with alpha instead of g}}
...
\end{align}
where we can seperate the variables
\begin{align}
    \frac{\dd\alpha_g}{\alpha_g^2}=\frac{\dd\ln(\mu)}{\mu}b_0\,.
\end{align}
...
\end{align}
where $\alpha_{g_0}=\alpha_g(\mu_0)$ and $\mu_0$ is a chosen RG reference scale. The running of the coupling is depicted in Figure \ref{fig:runningcoupling}, for the two non-trivial scenarios: $b_0>0$ and $b_0<0$. The former case implies a Landau pole at high energies, a characteristic of QED, and mathces the fate of the dashed green trajectory in Figure \ref{fig:QCDBeta}. The latter demonstrates asymptotically freedom, where the blue solid line converges to zero at high energies, illustrating QCD behaviour.   

The couplings can also be expressed as the inverse, where we will note the dependence on the energy scale explicitly
\begin{align}
    \alpha^{-1}_g(\mu)=\frac{1-b_0 \alpha_{g_0} \log \left(\frac{\mu}{\mu_0 }\right)}{\alpha_{g}(\mu_0)}=\alpha_g^{-1}(\mu_0)-b_0\log\frac{\mu}{\mu_0}\,,\label{eq:RGinversecoupling}
\end{align}
this equation will prove useful in Section \ref{chap:GUT}.

Now that we have reviewed the renormalisation group, beta functions, and asymptotic freedom, we can move onto discussing the Standard Model and the motivations of Grand Unified Theories. 