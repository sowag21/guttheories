...
The Georgi-Glashow model is considered, with $p=0$ and one scalar. The groups are thus \comment{unsure how to write it}
\begin{equation}
...
\begin{equation}
    \mathcal{L}_{kinetic}= -\frac{1}{4} F_{\mu\nu}^a F^{\mu\nu\,a}+i \bar \Psi_{ij}\slashed{D}\Psi_{ij}+i \bar \psi_i^a\slashed{D}\psi_i^a+(D_\mu \phi^*D^\mu\phi)^2\;,
\end{equation}
...
To analyze wether the theory is asymptotical free, we will use the Callan-Symanzik Equation \ref{}. The beta function will first be computed for the gauge coupling
\begin{equation}
    \mu\frac{\text{d}\alpha_g}{\text{d}\ln\mu}= b_0 \alpha_g^2\;,
\end{equation}
