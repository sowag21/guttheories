...
def count_sign_changes(coeffs):
    """
    Properly skips zeros to count sign changes.
    """
...
# --- Color Palettes ---
color_caf_flow = '#1f78b4'  # Dark Blue (On fixed flow)
color_caf_free = '#a6cee3'  # Light Blue (off fixed flow)

color_roots_4  = '#33a02c'  # Dark Green (4 roots)
color_roots_3  = '#4daf4a'  # Medium Green (3 roots)
color_roots_2  = '#b2df8a'  # Light Green (2 roots)
color_roots_1  = '#fdbf6f'  # Orange/Yellow (1 root)
color_roots_0  = '#e41a1c'  # Red (0 roots)

# ==========================================================
# LOOP OVER Ng VALUES (1 through 13)
# ==========================================================
for Ng in range(1, 14):
    print(f"Calculating and plotting GG Model for Ng = {Ng}...")
    
    with np.errstate(divide='ignore', invalid='ignore'):
        # ---------------------------------------------------------
        # 1. SHARED BASE PARAMETERS (GG Model)
        # ---------------------------------------------------------
        M_val = Ng * Nf - 4 * Ng + p
        ...
        # Upper sign evaluated for \mp: becomes -
    ...
    ax1.set_xlim(5, upNf); ax1.set_ylim(0, upp)
    ...
    leg1 = [#Patch(facecolor='none', edgecolor='none', label=fr'\textbf{{GG model:}} $N_g = {Ng}$'),
            #Patch(facecolor=color_roots_4, label='4 real roots'),
    ...
    ax1.legend(handles=leg1, loc='upper right', fontsize=22, frameon=True, 
               edgecolor='black', fancybox=False, title=fr'\textbf{{GG model ($N_g={Ng}$)}}', title_fontsize=24)
    
    plt.tight_layout()
    plt.savefig(f"CAFplotGG_1_RootsFixed_Ng{Ng}.pdf", format='pdf', bbox_inches='tight')
    plt.close(fig1)
    ...
    ax2.set_xlim(5, upNf); ax2.set_ylim(0, upp)
    ...
    leg2 = [#Patch(facecolor='none', edgecolor='none', label=fr'\textbf{{GG model:}} $N_g = {Ng}$'),
            #Patch(facecolor=color_roots_4, label='4 real roots'),
            Patch(facecolor=color_roots_2, label='2 real, 2 complex roots'),
            Patch(facecolor='white', edgecolor='black', label='4 complex roots'),
            Line2D([0], [0], color='black', linewidth=2.5, linestyle='dashed', label=r'$b_0=0$')]
    ...
    plt.tight_layout()
    plt.savefig(f"CAFplotGG_2_RootsNoY_Ng{Ng}.pdf", format='pdf', bbox_inches='tight')
    plt.close(fig2)


    # ==========================================================
    # PLOT 3: FINAL COMBINED CAF
    # ==========================================================
    ...
    ax3.contourf(Nf, p, intersection, levels=[0.5, 1.5], colors=[color_caf_flow], antialiased=True)
    
    # 4. Intersection Stripes: Overlay wide mathematical stripes ONLY in the intersection
    stripe_mask_wide = ((p + 3.0 * Nf) % 5) < 2
    striped_intersection = intersection & stripe_mask_wide
    ...
    ax3.set_xlim(5, upNf); ax3.set_ylim(0, upp)
    ...
    ax3.contour(Nf, p, Delta0_noY_analytical, levels=[0], colors="black", linewidths=3.0, linestyles='solid')
    ...
    ax3.legend(handles=leg3, loc='upper right', fontsize=22, frameon=True, 
               edgecolor='black', fancybox=False, title=fr'\textbf{{GG model ($N_g={Ng}$)}}', title_fontsize=24)

    plt.tight_layout()
    plt.savefig(f"CAFplotGG_3_FinalCombined_Ng{Ng}.pdf", format='pdf', bbox_inches='tight')
    plt.close(fig3)
    ...
    ax4.set_xlim(5, upNf); ax4.set_ylim(0, upp)
    ...
    leg4 = [Patch(facecolor='none', edgecolor='none', label=fr'\textbf{{GG model:}} $N_g = {Ng}$'),
    ...
    plt.tight_layout()
    plt.savefig(f"CAFplotGG_4_DescartesNoY_Ng{Ng}.pdf", format='pdf', bbox_inches='tight')
    plt.close(fig4)


print("All 20 GG plots generated successfully!")

# ==========================================================
# EXECUTION CODE
# ==========================================================
print("\nCalculating absolute minimums for 'On fixed flow' and 'alpha_y off fixed flow' (GG Model)...")
